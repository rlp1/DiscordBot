...
public class MuteCommand extends DiscordCommand {
    ...
    public void execute(final MessageReceivedEvent event, final String[] args) {
        ...
        try {
            event.getGuild().getController().setMute(selectedMember, true).queue();
        } catch (HierarchyException e) {
            event.getChannel().sendMessage("Você não tem permissão para mutar o usuário \"" + username + "\".")
                    .queue();
            return;
        }

        event.getChannel().sendMessage("\"" + event.getAuthor().getName() + "\" mutou o usuário \"" +
